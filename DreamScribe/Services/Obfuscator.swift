...
struct Obfuscator {
    ...
    static func getDeviceIdentifier() -> String {
        ...
        // Fallback to stored UUID. Read new key first; migrate from legacy VoiceInk key if present.
        let defaults = UserDefaults.standard
        let newKey = "DreamScribeDeviceIdentifier"
        let legacyKey = "VoiceInkDeviceIdentifier"
        if let storedId = defaults.string(forKey: newKey) {
            return storedId
        }
        if let legacyId = defaults.string(forKey: legacyKey) {
            defaults.set(legacyId, forKey: newKey)
            return legacyId
        }

        // Create and store new UUID
        let newId = UUID().uuidString
        defaults.set(newId, forKey: newKey)
        return newId
    }
