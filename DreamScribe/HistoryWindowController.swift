...
class HistoryWindowController: NSObject, NSWindowDelegate {
    ...
    private let windowIdentifier = NSUserInterfaceItemIdentifier("co.dreamersmedia.dreamscribe.historyWindow")
    private let windowAutosaveName = NSWindow.FrameAutosaveName("DreamScribeHistoryWindowFrame")

    private override init() {
        super.init()
    }

    func showHistoryWindow(modelContainer: ModelContainer, engine: DreamScribeEngine) {
    ...
    private func createHistoryWindow(modelContainer: ModelContainer, engine: DreamScribeEngine) -> NSWindow {
        ...
        window.title = "DreamScribe — Transcription History"
