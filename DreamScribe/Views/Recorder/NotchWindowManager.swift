...
class NotchWindowManager: ObservableObject {
    ...
    init(engine: DreamScribeEngine, recorder: Recorder) {
        guard let enhancementService = engine.enhancementService else {
            preconditionFailure("DreamScribeEngine.enhancementService must be non-nil when creating NotchWindowManager")
        }
