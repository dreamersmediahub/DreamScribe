...
class RecorderUIManager: ObservableObject {
    ...
    private weak var engine: DreamScribeEngine?
    private var recorder: Recorder?

    private let logger = Logger(subsystem: "co.dreamersmedia.dreamscribe", category: "RecorderUIManager")

    init() {}

    /// Call after DreamScribeEngine is created to break the circular init dependency.
    func configure(engine: DreamScribeEngine, recorder: Recorder) {
        self.engine = engine
